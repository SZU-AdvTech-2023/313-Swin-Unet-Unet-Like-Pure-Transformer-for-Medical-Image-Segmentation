...
#         return out
class PatchExpand(nn.Module):
    def __init__(self, input_resolution, in_channels, scale_factor,norm_layer=nn.LayerNorm):
        super().__init__()
        self.input_resolution = input_resolution
        self.dim = in_channels
        self.expand = nn.Linear(in_channels, 2*in_channels, bias=False) if scale_factor==2 else nn.Identity()
        self.norm = norm_layer(in_channels // scale_factor)
...
class BasicLayer_up(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size,
        ...
        if upsample is not None:
            self.upsample = PatchExpand(input_resolution, in_channels=dim, scale_factor=2)
        else:
            self.upsample = None
